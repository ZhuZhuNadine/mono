class Currencies {
  ...
  static fromISO4217(ISO4217){
    let currency
    for (const key in this.byCode){
      if (this.byCode[key] == ISO4217){
        currency = key
      }
    }
    if(currency){
...
function getCrossRate(currencyInfo, currencyIn){
  let crossRate
  if(currencyInfo.currencyCodeA === currencyIn){
...
// console.log(await currencyExchange(100,Currencies.byCode.USD,Currencies.byCode.EUR))
export { Currencies, getCrossRate }